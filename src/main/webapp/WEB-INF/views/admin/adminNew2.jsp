...
<div style="display: flex; border: 1px solid #D1D1D1;">
	...
	<div style="width: 60%;">
		<select id="optionLink" name="searchtype" onchange="if(this.value) location.href=(this.value);">
			...
			<option value="adNewlist.do?page=1" selected="selected">NEW</option>
...
<div id="newWrapper">
    <ul>	
                ...
                <c:forEach items="${ list }" var="nn">
                	<li>
                		<ul>
                      		<li><input type="checkbox"/></li>                			
                			<li>${ nn.rnum }</li>
                			<c:url var="newdt" value="/adHotnewdetail.do">
								<c:param name="hotnew_no" value="${ nn.hotnew_no }" />
								<c:param name="hotnew_no" value="${ nn.hotnew_no }" />
								<c:param name="hotnew_name" value="${ nn.hotnew_name }" />
								<c:param name="user_id" value="${ nn.user_id }" />
							</c:url>
                            <li class="left">
       ...
       <br>
        <!-- 페이징 처리 -->
		<c:import url="/WEB-INF/views/admin/newsearchpaging.jsp" />
    </div>
