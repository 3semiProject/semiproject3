...
<div style="text-align:center;"> <!-- 페이지 표시 영역 -->
	...
	<c:if test="${ currentPage > 1 }">
		<c:url var="p1" value="/adEyebodysearch.do">
	...
	<c:if test="${ (currentPage - 10) < startPage and (currentPage - 10) > 1 }">
		<c:url var="pbefore" value="/adEyebodysearch.do">
	...
	<c:forEach var="p" begin="${ startPage }" end="${ endPage }" step="1">
		...
		<c:if test="${ p ne currentPage }">
			<c:url var="pp" value="/adEyebodysearch.do">
	...
	<c:if test="${ (currentPage + 10) > endPage and (currentPage + 10) < maxPage }">
		<c:url var="pafter" value="/adEyebodysearch.do">
	...
	<c:if test="${ currentPage < maxPage }">
		<c:url var="pmax" value="/adEyebodysearch.do">
