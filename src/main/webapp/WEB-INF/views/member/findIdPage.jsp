...
function findIdPhone() {
	$.ajax({
		url: "findIdPhone.do",
		data: { name : $('#user_name').val(), phone : $('#phone').val()},
		type: "post",
		success: function(user_id) {
			if(user_id != "no") {
				alert("ID는 '" + user_id +"' 입니다.");
...
function findIdEmail() {
	$.ajax({
		url: "findIdEmail.do",
		data: { name : $('#user_name2').val(), email : $('#email').val()},
		type: "post",
		success: function(user_id) {
			if(user_id != "no") {
				alert("ID는 '" + user_id +"' 입니다.");
...
<div id="findEvBox">
<div id="clickID" onclick="clickID();" style="background-color: #BEE9C6">Find ID</div>
<div id="clickPW" onclick="clickPW();">Find PW</div>
</div>
...
<div class="phone_api">
<div><input type="tel" id="phone" value="휴대폰번호('-'없이 입력)" onfocus="this.value=''" />
...
<div class="sumbitBtn" onclick="findIdPhone();" >확인</div>
</form>
</div>
...
<form action="findIdEmail.do">
<div><input type="text" id="user_name2" value="${ user_name }" onfocus="this.value=''" /></div>
<div class="mail_api">
...
<div class="sumbitBtn" onclick="findIdEmail();" >확인</div>
</form>
</div>
</div>
</body>
