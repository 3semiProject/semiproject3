...
<div style="text-align:center;"> <!-- 페이지 표시 영역 -->
	...
	<c:if test="${ currentPage > 1 }">
		<c:url var="p1" value="/bannerlist.do">
	...
	<c:if test="${ (currentPage - 9.9) < startPage and (currentPage - 9.9) > 1 }">
		<c:url var="pbefore" value="/bannerlist.do">
	...
	<c:forEach var="p" begin="${ startPage }" end="${ endPage }" step="1">
		...
		<c:if test="${ p ne currentPage }">
			<c:url var="pp" value="/bannerlist.do">
	...
	<c:if test="${ (currentPage + 10) > endPage && maxPage - (endPage + 1) >= 0 }">
		<c:url var="pafter" value="/bannerlist.do">
	...
	<c:if test="${ currentPage < maxPage }">
		<c:url var="pmax" value="/bannerlist.do">
