...
<ul class="pagination modal">
	...
	<c:if test="${ currentPage > 1 }">
		<c:url var="p1" value="/qnasearch.do">
	...
	<c:if test="${ (currentPage - 9.9) < startPage and (currentPage - 9.9) > 1 }">
		<c:url var="pbefore" value="/qnasearch.do">
	...
	<c:forEach var="p" begin="${ startPage }" end="${ endPage }" step="1">
		...
		<c:if test="${ p ne currentPage }">
			<c:url var="pp" value="/qnasearch.do">
	...
	<!-- 다음 페이지그룹으로 이동하는 버튼 -->
	<c:if test="${ !((currentPage + 10) > endPage and maxPage - (endPage + 1) >= 0) }">
		<li><a class="arrow right">>></a></li>
	</c:if>
	<c:if test="${ (currentPage + 10) > endPage && maxPage - (endPage + 1) >= 0 }">
		<c:url var="pafter" value="/qnasearch.do">
	...
	<c:if test="${ currentPage < maxPage }">
		<c:url var="pmax" value="/qnasearch.do">
