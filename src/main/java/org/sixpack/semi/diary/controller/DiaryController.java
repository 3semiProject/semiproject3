...
public class DiaryController {
	...
	public String showFirstDiary( RedirectAttributes redirect,
		 ...
		 * 
		 * if (goal == null) { //조회값이 없거나, 목표일이 끝났을때 goal =
		 * diaryService.selectCurrentGoal(diary); System.out.println("goal1 : " + goal);
		 * 
		 * if (goal != null) { // 기한 내 목표 있을 경우 goalService.insertGoalInfo(goal); // 목표
		 * 생성 System.out.println("goal2 : " + goal);
		 * 
		 * } else { goal = diaryService.selectlastGoal(diary);
		 * System.out.println("goal3 : " + goal);
		 * 
		 * if (goal != null) { redirect.addAttribute("isExist", "D"); // 기간 내 목표가 없을 시
		 * redirect.addAttribute("goal", goal); return "redirect:diary_showGoal.do";
		 * //목표작성화면으로 이동 } else { redirect.addAttribute("isExist", "N"); // 처음 가입시(목표
		 * 데이터 없을 시) return "redirect:diary_showGoalModify.do"; //목표작성화면으로 이동 } } }
		 */
